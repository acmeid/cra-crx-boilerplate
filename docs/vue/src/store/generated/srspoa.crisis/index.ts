...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: srspoa.crisis initialized!')
		...
		async sendMsgVerifyInvariant({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SrspoaCrisis.tx.sendMsgVerifyInvariant({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgVerifyInvariant({ rootGetters }, { value }) {
			try {
				...
				const msg = await client.SrspoaCrisis.tx.msgVerifyInvariant({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgVerifyInvariant:Init Could not initialize signing client. Wallet is required.')
