...
import { Equivocation } from "srs-poa-client-ts/srspoa.evidence/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: srspoa.evidence initialized!')
		...
		async QueryEvidence({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SrspoaEvidence.query.queryEvidence( key.evidence_hash)).data
		...
		async QueryAllEvidence({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SrspoaEvidence.query.queryAllEvidence(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.SrspoaEvidence.query.queryAllEvidence({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgSubmitEvidence({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SrspoaEvidence.tx.sendMsgSubmitEvidence({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgSubmitEvidence({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SrspoaEvidence.tx.msgSubmitEvidence({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgSubmitEvidence:Init Could not initialize signing client. Wallet is required.')
