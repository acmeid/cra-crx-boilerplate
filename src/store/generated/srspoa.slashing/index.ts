...
import { SigningInfo } from "srs-poa-client-ts/srspoa.slashing/types"
import { ValidatorMissedBlocks } from "srs-poa-client-ts/srspoa.slashing/types"
import { MissedBlock } from "srs-poa-client-ts/srspoa.slashing/types"
import { ValidatorSigningInfo } from "srs-poa-client-ts/srspoa.slashing/types"
import { Params } from "srs-poa-client-ts/srspoa.slashing/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: srspoa.slashing initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SrspoaSlashing.query.queryParams()).data
		...
		async QuerySigningInfo({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SrspoaSlashing.query.querySigningInfo( key.cons_address)).data
		...
		async QuerySigningInfos({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SrspoaSlashing.query.querySigningInfos(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.SrspoaSlashing.query.querySigningInfos({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgUnjail({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SrspoaSlashing.tx.sendMsgUnjail({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgUnjail({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SrspoaSlashing.tx.msgUnjail({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUnjail:Init Could not initialize signing client. Wallet is required.')
